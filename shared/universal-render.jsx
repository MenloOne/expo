...
const runRouter = (location, routes) =>
  new Promise((resolve) => match({ routes, location }, (...args) => resolve(args)))

const bootstrap = () =>
  new Promise((resolve) =>
    Iso.bootstrap((initialState, container) => resolve({ initialState, container })),
  )
...
export default (async function({ flux, location }) {
  if (BROWSER) {
    ...
    const { activeLocale } = flux.getStore('locale').getState()
    const { messages } = await intlLoader(activeLocale)
    flux.getActions('locale').switchLocale({ locale: activeLocale, messages })
    ...
    const element = (
      <AltContainer flux={ flux }>
        <I18nContainer>
          <Router history={ withScroll(browserHistory) } routes={ routes(flux) } />
        </I18nContainer>
      </AltContainer>
    )
